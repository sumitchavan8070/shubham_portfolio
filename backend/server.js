...
//   // Execute Maven test
//   testExecutionProcess = spawn("mvn", ["test", `-Dsurefire.suiteXmlFiles=${testSuite}`], {
...
//   if (!responseSet) {
//     responseSet = true
//     res.json({ message: "Test execution started", testSuite, testName })
//   }
// })
...
app.get("/test-files", (req, res) => {
  ...
  try {
    ...
    const configPath = path.join(
      __dirname,
      "selenium-project/src/test/java/config/TestConfig.java"
    );
    let visibleTests = ["AmazonTest.java", "RedBusTest.java"]; // Default fallback
    ...
    if (fs.existsSync(configPath)) {
      const configContent = fs.readFileSync(configPath, "utf8");
      // Extract visible tests from config (simple parsing)
      const visibleTestsMatch = configContent.match(
        /VISIBLE_TESTS = Arrays\.asList$$([\s\S]*?)$$/
      );
      if (visibleTestsMatch) {
        visibleTests = visibleTestsMatch[1]
          .split(",")
      ...
      .map((file) => {
        ...
        if (fs.existsSync(configPath)) {
          ...
          const descMatch = configContent.match(
            new RegExp(`case "${file}":[\\s\\S]*?return "([^"]+)"`)
          );
          if (descMatch) {
            description = descMatch[1];
          }
    ...
    const files = fs
      ...
      .map((file) => {
        ...
        switch (file) {
          case "ExcelUtils.java":
            description =
              "Excel file operations - Read/Write data, cell formatting, data-driven testing support";
            break;
          case "ActionUtils.java":
            description =
              "Selenium Actions utility - Mouse hover, drag & drop, keyboard shortcuts, element interactions";
          ...
          case "WaitUtils.java":
            description =
              "Custom wait strategies - Element visibility, page load, dynamic content handling";
            break;
          case "ScreenshotUtils.java":
            description =
              "Screenshot utilities - Full page capture, element screenshots, failure screenshots";
            break;
        }
...
app.post("/execute-test", (req, res) => {
  if (runningTests.size > 0) {
    return res
      .status(400)
      .json({ error: "Test execution already in progress" });
  }
  ...
  // Execute Maven test
  const testExecutionProcess = spawn(
    "mvn",
    ["test", `-Dsurefire.suiteXmlFiles=${testSuite}`],
    {
      cwd: path.join(__dirname, "selenium-project"),
      stdio: ["pipe", "pipe", "pipe"],
    }
...
app.listen(PORT, () => {
  console.log(`Server running on port ${PORT}`);
  console.log(
    `Test reports available at: http://localhost:${PORT}/test-reports`
  );
});
