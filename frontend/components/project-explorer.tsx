...
//         const setDefaultExpanded = (nodes: FileNode[], path = "") => {
//           nodes.forEach((node) => {
//             const currentPath = `${path}/${node.name}`;
//             if (node.type === "folder") {
//               defaultExpanded[currentPath] = true;
//               if (node.children) {
//                 setDefaultExpanded(node.children, currentPath);
//               }
//             }
//           });
//         };
//         loaded.forEach((project) => {
//           setDefaultExpanded(project.structure);
//         });
...
//     const currentPath = `${path}/${node.name}`;
//     if (node.type === "folder") {
//       const isExpanded = expandedFolders[currentPath] ?? true; // Default to true if not set
//       return (
//         <div key={currentPath} className="pl-2 sm:pl-4">
//           <div
//             className="flex items-center py-1 hover:bg-muted dark:hover:bg-muted/30 cursor-pointer rounded-md"
          ...
          {isExpanded && node.children && (
            <div className="space-y-1">
              ...
              {node.children
                ...
                .map((child) => renderFileNode(child, currentPath))}
              {/* Then render files */}
              {node.children
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-6">
      ...
      <div className="flex flex-col sm:flex-row h-[calc(100vh-180px)] sm:h-[600px] border rounded-lg overflow-hidden bg-card dark:bg-card text-foreground shadow-sm">
        {/* File Tree - Full width on mobile, fixed width on desktop */}
        <div className="sm:w-64 w-full border-r bg-background overflow-y-auto">
          <div className="p-2 sm:p-3 text-xs sm:text-sm font-mono text-muted-foreground space-y-1">
            {/* Sort root level items - folders first */}
